...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run layer-wise hierarchical multi-superquadric recovery with 4-parameter CMA-ES search "
    ...
    parser.add_argument("--EpsInit", type=float, default=1.7)
    parser.add_argument("--MinPointsInit", type=float, default=60.0)

    # CMA-ES config
    parser.add_argument("--popsize", type=int, default=20)
    parser.add_argument("--maxiter", type=int, default=30)
    parser.add_argument("--cmaSigma", type=float, default=0.15)
...
def main(argv: list[str]) -> None:
    ...
    init_particle = np.array([args.OutlierRatioInit, args.SigmaInit, args.EpsInit, args.MinPointsInit], dtype=float)

    start = timeit.default_timer()
    result = run_layerwise_cmaes(
    ...
    if args.out:
        output = {
            ...
            "cmaes_config": asdict(cma_cfg),
            "initial_values": {
                "OutlierRatioInit": float(args.OutlierRatioInit),
                "SigmaInit": float(args.SigmaInit),
                "EpsInit": float(args.EpsInit),
                "MinPointsInit": float(args.MinPointsInit),
            },
            "objective_config": {
                "fitness_mode": args.FitnessMode,
