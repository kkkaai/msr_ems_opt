...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run single-superquadric recovery with PSO tuning on (OutlierRatio, Sigma)."
    )
    ...
    # Global normalization (external to EMS)
    ...
    # Fixed EMS settings (keep aligned with external/Python/tests/multiquadric_test.py)
    ...
    # PSO search space (optimized params)
    ...
    # PSO config
    parser.add_argument("--swarmsize", type=int, default=10)
...
def main(argv: list[str]) -> None:
    ...
    init_particle = np.array([args.OutlierRatioInit, args.SigmaInit], dtype=float)
    pso_cfg = PsoConfig(
    ...
    def evaluate_candidate(
        x: np.ndarray,
    ) -> tuple[float, object | None, np.ndarray | None, float, float]:
    ...
    def objective(x: np.ndarray) -> float:
        fitness, _, _, _, _ = evaluate_candidate(x)
        return fitness

    start = timeit.default_timer()
    best_x, best_f, history = run_pso(
        objective=objective,
        lb=lb,
        ub=ub,
        config=pso_cfg,
    ...
    best_f_eval, best_sq, best_p, best_d_fit, best_cov = evaluate_candidate(best_x)
    stop = timeit.default_timer()

    if best_sq is None or best_p is None:
        raise RuntimeError("PSO finished but no valid superquadric was recovered.")
    ...
    best_outlier_ratio = float(np.clip(best_x[0], 0.0, 0.999))
    best_sigma = max(0.0, float(best_x[1]))
    inlier_count = int(np.sum(best_p > args.pThreshold))
    outlier_count = int(point_working.shape[0] - inlier_count)

    print(f"Best fitness: {best_f_eval:.6f}")
    ...
    if args.out:
        output = {
            ...
            "pso_history": [float(v) for v in history],
