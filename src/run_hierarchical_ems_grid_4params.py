...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Uniform divisions per dimension. 5 => 5^4 candidates per layer.",
    )
...
def main(argv: list[str]) -> None:
    ...
    lb = np.array(
        [args.OutlierRatioMin, args.SigmaMin, args.EpsMin, args.MinPointsMin],
        dtype=float,
    )
    ub = np.array(
        [args.OutlierRatioMax, args.SigmaMax, args.EpsMax, args.MinPointsMax],
        dtype=float,
    )
    if np.any(lb >= ub):
    ...
    if args.result:
        payload = {
            "fitness_details": result.get("fitness_details", {}),
            "layer_best_hyperparameters": result["layer_best_hparams"],
            "layer_best_fitness": result["layer_best_fitness"],
            "layer_best_distance_fit": result.get("layer_best_distance_fit", []),
            "layer_best_coverage_ratio": result.get("layer_best_coverage_ratio", []),
            "layer_best_residual_unexplained_ratio": result.get("layer_best_residual_unexplained_ratio", []),
            "layer_best_num_superquadrics": result.get("layer_best_num_superquadrics", []),
